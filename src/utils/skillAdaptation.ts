...
export function buildSkillAdaptationPrompt(
  ...
  if (language === "zh") {
    return `请作为 9X bot 技能适配工程师，对以下来自「${marketLabel}」的技能包进行全面兼容性检查与适应性改写，并直接开始执行（无需等待我确认）。
...
- 改写后的 SKILL.md 必须符合 9X bot skill_manage 格式（frontmatter 含 name、description、tools 等）。
- 不要编造本机不存在的工具或 MCP；缺失能力须在验收清单中明确标注。`;
  }

  return `Act as a 9X bot skill adaptation engineer. Perform a full compatibility review and adaptive rewrite for the following bundle from the **${marketLabel}**, then start executing immediately (no need to wait for my confirmation).
...
- Output SKILL.md must remain valid for 9X bot skill_manage.
