...
本 Skill 面向普通办公用户。优先使用 9X bot 已内置或当前环境已可用的能力完成 SQL 生成、解释、优化和数据分析；不要要求用户自行安装 Python、pip、命令行工具或依赖包。

### 环境处理原则

- 如果完整数据执行能力可用，直接完成查询、分析和导出。
- 如果运行环境缺少依赖，使用“无依赖模式”输出可复制的 SQL、分析思路、表结构建议、排错步骤或轻量结果。
- 如确实无法继续，向用户说明缺少什么能力，并给出非技术同事能理解的下一步，而不是暴露安装命令。
