...
5. **⚠️ 错误处理与重试限制**：每个脚本调用最多重试 **1 次**（换参数或换调用方式）。如果 2 次仍失败（如返回 "Mock not found"、空结果、报错），**立即跳过该步骤，用已有数据继续推进到下一步直至步骤 7**。严禁反复重试同一脚本、读取脚本源码、或切换到非脚本方式（如 `tencent-ads api`）尝试绕过。宁可在最终请求中缺少某个字段，也不要耗尽所有轮次。
...
```json
{
  "success": true,
  "adgroup_id": <integer>,
  "summary_text": "<由脚本拼装的单句中文回执，包含创建关键信息>"
}
```
...
> ⛔ **创建成功后必须将 `summary_text` 原样输出给用户**（含其中的换行）。该字段已由脚本拼装好首行标题 + 多行列表的中文回执，**不要省略字段、不要改写、不要翻译、不要重新排版（不要改成表格或合并成单段），也不要参考其中具体数值作为后续推理依据**。失败时无 `summary_text`，按 `error.message_cn` 与 `trace_id` 向用户说明原因。
