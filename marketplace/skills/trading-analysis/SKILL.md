...
本技能依赖 `finance-data-retrieval` 技能获取市场数据。在 Phase 1 数据收集阶段，所有市场数据通过该技能调用 Tushare Pro API 获取。新闻和情绪数据使用 web_search 搜索。
...
1. **公司新闻**：使用 web_search 搜索标的公司的最新新闻（近 1-2 周）
2. **行业新闻**：搜索所属行业的最新动态
...
1. **社交媒体**：使用 web_search 搜索标的公司在雪球、东方财富股吧、微博等平台的讨论
...
2. **新闻和情绪**：使用 web_search 工具搜索，可辅助使用 `finance-data-retrieval` 的新闻接口（`news`、`major_news`）
